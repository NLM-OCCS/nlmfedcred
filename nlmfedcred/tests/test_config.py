...
def test_none_none(tmpdir):
    ...
    c = parse_config(None, None, None, None, None, inipath=str(inipath))
...
def test_file_not_found(tmpdir):
    ...
    c = parse_config(None, 99999, 'nlm_aws_users', 'authtest.nih.gov', None, inipath=str(inipath))
...
def test_realistic(tmpdir):
    ...
    c = parse_config('NLM-QA',  None, None, None, None, inipath=str(inipath))
    ...
    c = parse_config('NLM-INT', None, 'nlm_aws_admin', None, None, inipath=str(inipath))
...
def test_realistic_profilenotfound(tmpdir):
    ...
    with pytest.raises(ProfileNotFound):
        parse_config('nosuchprofile', None, None, None, None, inipath=str(inipath))
...
def test_ca_bundle_none_none(tmpdir, mocker):
    ...
    with mocker.patch('nlmfedcred.config.get_aws_config_path', return_value=awsconfig):
        c = parse_config('default', None, None, None, None, ca_bundle=None)      # being explicit
        assert c.ca_bundle is None
...
def test_ca_bundle_takes_aws_over_none(tmpdir, mocker):
    ...
    with mocker.patch('nlmfedcred.config.get_aws_config_path', return_value=awsconfig):
        c = parse_config('default', None, None, None, None)
...
def test_ca_bundle_takes_provided_over_aws(tmpdir, mocker):
    ...
    with mocker.patch('nlmfedcred.config.get_aws_config_path', return_value=awsconfig):
        c = parse_config('default', None, None, None, None, ca_bundle='override-bundle')
